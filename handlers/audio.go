...
// 		if err != nil {
// 			log.Println("Error decoding slide image")
// 			sendSSE("Error decoding slide image")
// 			return
// 		}

// 		order := slideImage["order"].(int32)
// 		sendSSE(fmt.Sprintf("Generating audio for slide image with order %d", order))

// 		audioURL, ok := slideImage["audio_url"].(string)
// 		if ok && audioURL != "" {
// 			sendSSE(fmt.Sprintf("Audio URL already exists for order %d", order))
// 			continue
// 		}

// 		err = generateAudioForSlideImage(ctx, slideImage, sendSSE)
// 		if err != nil {
// 			sendSSE(fmt.Sprintf("Error generating audio for slide image with order %d: %s", order, err.Error()))
// 			return
// 		}
// 	}

// 	sendSSE("Audio generated for all slide images")
// 	c.JSON(http.StatusOK, gin.H{"status": "success", "message": "Audio generated for all slide images"})
// }

// func generateAudioForSlideImage(ctx context.Context, slideImage bson.M, sendSSE func(message string)) error {
// 	generatedText, ok := slideImage["generated_text"].(string)
// 	if !ok {
// 		sendSSE("Generated text not found")
// 		return errors.New("generated text not found")
// 	}
...
// 	sendSSE(fmt.Sprintf("Slide image updated with audio URL: %s", audioURL))
// 	return nil
// }
